...
# THFE 1.0 windows compatible

This version of TFHE 1.0 provides a windows compatible build. 
Currently only the FFTW version of tfhe is supported. The reason is the asm part of the fma and avx FFT implementations.

## Build

For a build you need a minGW32 environment, cmake and the fftw3 library installed.

### 1. Download the FFTW Lib 

Got to http://www.fftw.org/download.html and download the prebuilt Windows binaries. Copy the libfftw3-3.dll to libs folder.

> Building FFTW3 from source is also possible. A tutorial is available at http://www.fftw.org/.
...
cmake --install
```
### 3. Compile own Applications

At example folder you can find an example application using tfhe. Because miniGW is used as compiler and some featues of libstd are used, some DLLS of MiniGW are needed to run the builded executable.
The following libraries are needed to run the application:

* libfftw3-3.dll (from fftw3)
* libgcc_s_seh-1.dll (from minigw folder)
* libstdc++-6.dll (from minigw folder)
* libtfhe-fftw.dll (from tfhe build folder)
* libwinpthread-1.dll (from minigw folder)



Below, the original tfhe readme is attached.
